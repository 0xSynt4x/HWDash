...
export const Panel = ({ children, className = '' }: { children: React.ReactNode; className?: string }) => (
  <div className={`flex flex-col mb-1 bg-[var(--glass-bg)] backdrop-blur-md border border-[var(--glass-border)] rounded-md p-2 shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:border-[var(--border-hover)] transition-all duration-300 ${className}`}>
    {children}
  </div>
);
      ...
      <div className="flex items-center gap-2 flex-1">
        ...
        {isEditing ? (
          ...
          <span
            ...
            className={onTitleChange ? "cursor-text hover:text-[var(--text-primary)] transition-colors font-semibold" : "font-semibold"}
          >
            {title}
          </span>
        )}
...
export const DynamicLineGraph = ({ cpuPower = 0, gpuPower = 0, fps = 0 }: { cpuPower?: number, gpuPower?: number, fps?: number }) => {
  const [history, setHistory] = useState<{cpu: number, gpu: number, fps: number}[]>(Array(60).fill({cpu: 0, gpu: 0, fps: 0}));
    ...
    <div className="relative w-full h-full">
      ...
      <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
        {[0, 25, 50, 75, 100].map(y => (
          <line key={y} x1="0" y1={`${y}%`} x2="100%" y2={`${y}%`} stroke="var(--border-color)" strokeWidth="0.5" opacity="0.3" strokeDasharray="2,2" />
        ))}
      </svg>
      ...
      <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
        ...
        <circle cx="99" cy={100 - (Math.min(history[history.length-1]?.cpu || 0, maxValues.cpu) / maxValues.cpu) * 100} r="2" fill="var(--chart-cpu)" opacity="0.8" />
      </svg>
    </div>
  );
};
